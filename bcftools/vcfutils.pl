...
sub varFilter {
  my %opts = (d=>2, D=>10000000, a=>2, W=>10, Q=>10, w=>3, p=>undef, 1=>1e-4, 2=>1e-100, 3=>0, 4=>1e-4, G=>0, S=>1000, e=>1e-4, f=>0);
  getopts('pd:D:W:Q:w:a:1:2:3:4:G:S:e:f:', \%opts);
  die(qq/
...
Options: -Q INT    minimum RMS mapping quality for SNPs [$opts{Q}]
         ...
         -f FLOAT  minimum variation frequency (between 0 and 1) [$opts{f}]
...
\n/) if (@ARGV == 0 && -t STDIN);
  ...
  while (<>) {
	...
	my $flt = 0;
	# parse annotations
	my ($dp, $mq, $dp_alt, $varfreq) = (-1, -1, -1, -1);
